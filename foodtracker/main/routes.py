...
def create_log():
    ...
    log = Log(data=datetime.strptime(date, '%Y-%m-%d'))
    db.session.add(log)
    db.session.commit()
...
def add_food_to_log(log_id):
    log = Log.query.get_or_404(log_id)
    select_food = request.form.get('food-select')
    food = Food.query.get(int(select_food))
